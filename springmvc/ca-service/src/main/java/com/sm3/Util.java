...
public class Util {
	...
	//两个合并字节
	public static byte[] TwoByteMerger(int s1_length, byte[] s1, 
			int s2_length, byte[] s2) {
		byte[] s = new byte[s1_length + s2_length];
		System.arraycopy(s1, 0, s, 0, s1_length);
		System.arraycopy(s2, 0, s, s1_length, s2_length);
		return s;
	}
