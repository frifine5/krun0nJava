...
//        FileUtil.writeInFiles("C:\\Users\\49762\\Desktop\\cert1.cer", cert.getEncoded());
        ...
        ASN1Encodable[] attDigestSet = {PKCSObjectIdentifiers.pkcs_9_at_messageDigest, new DERSet(new DEROctetString(md))};
        DERSequence attDigest = new DERSequence(attDigestSet);
        ASN1Encodable[] attOtherSet = {PKCSObjectIdentifiers.pkcs_9_at_extensionRequest, new DERSet(new DEROctetString(esealByt))};
        DERSequence attSeqOther = new DERSequence(attOtherSet);

        ASN1EncodableVector attVect = new ASN1EncodableVector();
        attVect.add(attSeq1);
        attVect.add(attSeq2);
        attVect.add(attDigest);
        attVect.add(attSeqOther);
        ...
        System.out.println(contentInfoSeqRoot.getEncoded().length);
    ...
    @Test
    public void testSignOnPdf1()throws Exception{


        String dir = "C:\\Users\\49762\\Desktop\\";
        String pdfName = "1.pdf";
        String pdfDest = "si-1.pdf";

        String orgStamp = "/home/dtmp/itextPdfs/orgStamp.png";
        String pdf = dir + pdfName;
        String fieldName = "sign1";

        byte[] imageData = FileUtil.fromDATfile( orgStamp);
...
//        appearance.setLocation(location);
        ...
        appearance.setVisibleSignature(new Rectangle(150, 400, 230, 480 ), 1,  fieldName);




        // 调整图片大小
        InputStream ss = new ByteArrayInputStream(imageData);
        BufferedImage bi = ImageIO.read(ss);
        BufferedImage nbi = TestPdfSIgnDoc.resizeImage(bi, (float)80/(float)80);
        ...
        Image signImage = Image.getInstance(imageData);
...
//        FileUtil.writeInFiles("C:\\Users\\49762\\Desktop\\cert1.cer", cert.getEncoded());
        ...
        byte[] md0 = outputStream.toByteArray();
        ...
        byte[] sigData = contentInfoSeqRoot.getEncoded();

        byte[] finalSigData = new byte[sigDataLen];
        System.arraycopy(sigData, 0, finalSigData, 0, sigData.length);
        for(int i = sigData.length; i< sigDataLen; i++){
            finalSigData[i] = 0x0;
        }

        PdfDictionary dic2 = new PdfDictionary();
        dic2.put(PdfName.CONTENTS, new PdfString(finalSigData).setHexWriting(true));
        appearance.close(dic2);

        byte[] destByts = result.toByteArray();


        if(null != destByts){
            FileOutputStream fos = new FileOutputStream(dir + pdfDest);
            int len = destByts.length;
            fos.write(destByts, 0, len);
            fos.flush();
            fos.close();
        }
    ...
    @Test
    public void testPdfIsFinal()throws Exception{
        String dir = "C:\\Users\\49762\\Desktop\\";
        String pdfName = "si-1.pdf";
        String pdf = dir + pdfName;
        PdfReader pdfReader = new PdfReader(pdf);
...
//        Map<String, AcroFields.Item> fields = acreFileds.getFields();
        ...
        for(String sigName : sigNames){
            boolean isLastSign = acreFileds.signatureCoversWholeDocument(sigName);
            System.out.println(sigName + "签名保护内容是否覆盖全文:" + isLastSign);
        }
