...
//      System.out.println("C3 " + Util.byteToHex(c3));
    ...
    //数据解密
    public static byte[] decrypt(byte[] privateKey, byte[] encryptedData, boolean hasLen) throws IOException {
        if (privateKey == null || privateKey.length == 0) {
            return null;
        }
        ...
        byte[] c2;
        if(hasLen){
            c2 = StrUtil.hexToByte(data.substring(130 + 64 + 8));
        }else{
            c2 = StrUtil.hexToByte(data.substring(130 + 64));
        }
...
//        prik = "1C2238FA4B99D89BC9C5930EA0A2AFFAEB5D47B63FDC53036E529A62A9FE52B6";
        ...
        String decText = new String(SM2EncUtil.decrypt(StrUtil.hexToByte(prik), StrUtil.hexToByte(cipherText), false));
