...
this.color={cColor:"#fff",bColor:"#FFFFFF",tColor:"#d8e4f6",wColor:"#996935"};
...
var myelements = document.getElementsByTagName('script');
var myPath = '';
for (var i = 0, len = myelements.length; i < len; i++) {
	if (myelements[i].src && myelements[i].src.match(/popup[\w\-\.]*\.js/)) {
		if(myelements[i].src.indexOf("http://")!=-1){
		    var num1=myelements[i].src.indexOf("//")
		    var num2=myelements[i].src.indexOf("/",num1+3);
			myPath=myelements[i].src.substring(num2+1,myelements[i].src.length);
			myPath="/"+myPath.substring(0,myPath.indexOf("/"),1);
			break;
		}else{
			myPath = myelements[i].src.substring(0, myelements[i].src.indexOf('/',1));
			break;
		}
	}
}
...
//    if (elements[i].src && elements[i].src.match(/jquery[\w\-\.]*\.js/)) {
//    	mypath = elements[i].src.substring(0, elements[i].src.lastIndexOf('/',1) );
//    }
//}
var path= "/nisec_RC/css/images/tooltip/";
...
isOpenOrClose=true;
//页面帐号管理页面生成主账号页关闭设置按钮不可用 --李平
$("#zhanghaoguanli").attr('disabled','true');
$("#sezhiiamyonghu").attr('disabled','true');
this.showSome(); 
//防止刷新页面-因此注释
